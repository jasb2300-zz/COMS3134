/**
 * This is the main java file for the programming portion of Homework 5.
 * Jorge Solis
 * jas2430
 * May 1st, 2017
...
public class Dijkstra {
   ...
   */
    public void computeAllEuclideanDistances()
    {
        ...
        for(Vertex v : vertices)
        {
            ...
            {
                double ux = e.source.x;
                double uy = e.source.y;
                double vx = e.target.x;
                double vy = e.target.y;
                e.distance = computeEuclideanDistance(ux, uy, vx, vy);
            }
        }
    }
   ...
   */
    ...
    {
        Collection<Vertex> unknownVertices = new LinkedList<Vertex>();
        ...
        Vertex nearest = this.getVertex(s);
        nearest.distance = 0;
        unknownVertices.add(nearest);
        //System.out.println(nearest.name);
        //System.out.println(nearest.distance);
        while(!unknownVertices.isEmpty())
        {
            ...
            nearest.known = true;
            unknownVertices.remove(nearest);
            for(Edge e : nearest.adjacentEdges)
            {
                //System.out.println(e.distance);
                //System.out.println(e.source);
                //System.out.println(e.target);
                //System.out.println(e.target.distance);
                //System.out.println(e.target.name);
                if(nearest.distance + e.distance < e.target.distance)
                {
                    e.target.distance = nearest.distance + e.distance;
                    unknownVertices.add(e.target);
                    e.target.prev = nearest;
                    //System.out.println(nearest);
                    e.target.prev = nearest;
                    //System.out.println(e.target.name);
                    //System.out.println(e.target.prev);
                }
            }
        }
    }
   ...
   */
    ...
    {
        doDijkstra(s);
        List<Edge> hops = new LinkedList<Edge>();
        Stack<Vertex> stops = new Stack<Vertex>();
        Vertex destination = this.getVertex(t);
        System.out.println("Total distance: "+ destination.distance);
        //System.out.println(t);
        //System.out.println(destination);
        //System.out.println(destination.prev);
        for(stops.push(destination); destination.prev != null; stops.push(destination))
        {
        ...
        while(stops.size() > 1)
        {
            Vertex v = stops.pop();
            Vertex w = stops.peek();
            Edge hop = v.adjacentEdges.get(0);
            for(Edge e : v.adjacentEdges)
            {
                //System.out.print(e.target.name);
                if(e.target.equals(w))
                    hop = e;
            }
            hops.add(hop);
        }
